        ...
        <div class="bg-white rounded-lg shadow pt-1 pb-2">
            <table class="w-full">
                ...
                <tr class="border-b border-gray-300">
                    ...
                    <th class="text-left pl-2 pr-6 pt-2 pb-2 font-normal">{{ __('Finished at') }}</th>
                </tr>
                </thead>
                ...
                @foreach ($workflows as $workflow)
                    <tr class="text-sm transition duration-300 hover:bg-gray-100 cursor-pointer text-gray-800">
                        ...
                        <td class="px-2 py-2">
                            <a href="{{ route('venture.dashboard.workflows.show', $workflow) }}" class="text-laravel">
                                {{ $workflow->name }}
                            </a>
                        </td>
                        <td class="pl-2 pr-6 py-2">{{ $workflow->created_at->format('Y-m-d H:i') }}</td>
                        <td class="pl-2 pr-6 py-2">{{ Carbon\Carbon::parse($workflow->finished_at)->format('Y-m-d H:i') }}</td>
                    </tr>
                @endforeach
                </tbody>
            </table>
